...
Public Module Win32
	...
	Public Declare Function CommandLineToArgvW Lib "shell32.dll" (ByVal lpCmdLine As String, ByRef pNumArgs As Integer) As IntPtr
	Public Function CommandLineToArgs(ByVal lpCmdLine As String) As String()
		Dim inQuotes As Boolean = False
		Dim parmChars = lpCmdLine.ToCharArray
		For i = 0 To parmChars.Count - 1
			If parmChars(i) = """" Then inQuotes = Not inQuotes
			If (Not inQuotes) And parmChars(i) = " " Then parmChars(i) = vbCr
		Next
		Return New String(parmChars).Split(vbCr)
	End Function

	Public Function GetCommandLineOfProcess(proc As Process) As String
		Using searcher As Management.ManagementObjectSearcher = New Management.ManagementObjectSearcher("SELECT CommandLine FROM Win32_Process WHERE ProcessId = " & proc.Id)
			Using objects As Management.ManagementObjectCollection = searcher.Get
				Return objects.Cast(Of Management.ManagementBaseObject)().SingleOrDefault()?("CommandLine")?.ToString()
			End Using
		End Using
	End Function
